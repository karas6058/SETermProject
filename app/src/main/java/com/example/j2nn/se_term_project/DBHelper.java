...
public class DBHelper extends SQLiteOpenHelper{
    ...
    public void deleteMovie(String name) {
        SQLiteDatabase db = getWritableDatabase();
        if (db.delete("MOVIE", "movieName = ?", new String[] { name }) == 1)
            Log.d("######", "delete movie:"+name);
        else
            Log.d("######", "Do not delete movie:"+name);
    ...
    public ArrayList<String> info() {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT movieName FROM MOVIE", null);
    ...
    public ArrayList<String> detail(String name) {
        ArrayList<String> movie = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT movieName, grade, director, actor, runningTime, movieImg, desc FROM MOVIE WHERE movieName='" + name + "'", null);
    ...
    public void update(String title, String grade, String director, String actors, int runningTime, String desc, String poster) {
        SQLiteDatabase db = getReadableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put("movieName", title);
        contentValues.put("grade", grade);
        contentValues.put("director", director);
        contentValues.put("actor", actors);
        contentValues.put("runningTime", runningTime);
        contentValues.put("movieImg", desc);
        contentValues.put("desc", poster);

        db.update("MOVIE", contentValues, "movieName = ?", new String[] {title});
